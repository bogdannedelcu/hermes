...
//>> Properties
    ...
    */
    ...
    /**
    * Sets the loadCompleted event of the TreeView.
    * This event triggers only when loadOnDemand is set to false - it indicates that all nodes that needs to be loaded are ready and present as data items in the DataSource of the TreeView.
    * @param string|\Kendo\JavaScriptFunction $value Can be a JavaScript function definition or name.
    * @return \Kendo\UI\TreeView
    */
    public function loadCompleted($value) {
        if (is_string($value)) {
            $value = new \Kendo\JavaScriptFunction($value);
        }

        return $this->setProperty('loadCompleted', $value);
    }
