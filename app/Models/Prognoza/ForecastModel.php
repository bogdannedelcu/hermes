...
class ForecastModel extends Model
{
	use \MasterDataTools;
	use \CacheTools;
	use \App\Models\Prognoza\Algorithms\V1Algorithm;
	use \App\Models\Prognoza\Algorithms\V2Algorithm;
	use \App\Models\Prognoza\Algorithms\V3Algorithm;
	use \App\Models\Prognoza\Algorithms\V4Algorithm;

	// Master switch for the experimental algorithms. Production runs v1 only — v2/v3/v4
	// stay in the code so we can run on-demand benchmarks, but estimate() does not invoke
	// them automatically. Set to true (here or via setRunExtraAlgorithms(true)) when you
	// want a full multi-algorithm ablation pass.
	private $runExtraAlgorithms = false;

	public function setRunExtraAlgorithms($enabled)
	{
		$this->runExtraAlgorithms = (bool)$enabled;
		return $this;
	}
	...
	{	
		...
		for($idx=0;$idx<$modifiedData->length;$idx++)
		{
			...
			if($removeFromManualMode)
			{
				...
				$this->writeData('DELETE FROM forecast_estimates WHERE supplier_id = '.$this->supplierID.' AND customer_id = '.$customerID.' AND forecast_datetime >= "'.$dt.'" AND forecast_datetime < DATE_ADD("'.$dt.'", INTERVAL 1 DAY)');
			}
			else
			{
	...
	public function uploadValues($customerID, $YMdate, $values, $startDate, $endDate, $partial)
	{
		...
		foreach($deletedDays as $d)
		{
			...
			$dt2 = $YMdate.'-'.sprintf('%02d',$d);
			$this->writeData('DELETE FROM forecast_estimates WHERE supplier_id = '.$this->supplierID.' AND customer_id = '.$customerID.' AND forecast_datetime >= "'.$dt2.'" AND forecast_datetime < DATE_ADD("'.$dt2.'", INTERVAL 1 DAY)');
		}
	...
	public function readEstimates($month, $year, $customers, $consumptionType,$intervalType)
	{
		...
		$dateFrom = "$year-" . sprintf('%02d', $month) . "-01";
		...
		if(!empty($customers))
		{
			$sql = "SELECT DAY(fc.forecast_datetime) as day, HOUR(fc.forecast_datetime) as hour, minute(fc.forecast_datetime) as minute, SUM(forecast_ea) AS forecast_ea FROM forecast_estimates fc
					WHERE fc.forecast_datetime >= '$dateFrom' AND fc.forecast_datetime < '$dateTo' AND customer_id IN ($customerIDs) AND supplier_id=$supplierID AND fc.algorithm = 'v1'
			...
			$sql = "select day(fc.forecast_datetime) as day, hour(fc.forecast_datetime) as hour ,minute(fc.forecast_datetime) as minute, sum(forecast_ea) as forecast_ea from forecast_estimates fc
			 JOIN (SELECT ct.customer_id, MIN(sr.start_date), MAX(ct.contract_stop) from contracts ct
				  ...
				  WHERE '$dateFrom'>=sr.start_date AND (ct.contract_stop IS NULL OR ct.contract_stop >= '$dateFrom')
			      GROUP BY ct.customer_id ) A ON A.customer_id = fc.customer_id
				  where fc.forecast_datetime >= '$dateFrom' AND fc.forecast_datetime < '$dateTo' AND supplier_id=$supplierID AND fc.algorithm = 'v1'
				  $cWhere
			group by $dateGroup order by hour(forecast_datetime), $minOrder day(forecast_datetime)";
	...
	public function readAllEstimates($eStart, $eEnd, $customers,$consumptionTypeID, $intervalType, $algorithm = 'v1')
	{
		$supplierID = $this->supplierID;
		$customerIDs = implode(',', array_map('intval', $customers));
		...
		if($intervalType == "15min")
		{
			// 15min: no aggregation needed — one row per interval, use inner subquery to hit the composite index
			$sql = "SELECT c.customer_id, c.customer_name, fe_agg.forecast_datetime, fe_agg.forecast_ea FROM customers c
					...
					LEFT JOIN (
						SELECT customer_id, forecast_datetime, forecast_ea
						FROM forecast_estimates
						WHERE supplier_id = $supplierID AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = '$algorithm'
					) fe_agg ON fe_agg.customer_id = c.customer_id
					$joinCT
					WHERE c.customer_status = 'Activ' AND c.supplier_id = $supplierID $cWhr
					ORDER BY HOUR(fe_agg.forecast_datetime), MINUTE(fe_agg.forecast_datetime), vco.fctOrder, c.customer_name, DAY(fe_agg.forecast_datetime)";
		}
		else
		{
			// hourly: aggregate in inner subquery using the composite index, then join for customer metadata
			$sql = "SELECT c.customer_id, c.customer_name, fe_agg.forecast_datetime, fe_agg.forecast_ea FROM customers c
					JOIN forecast_view_customer_order vco on c.customer_id = vco.customer_id
				    JOIN (SELECT ct.customer_id FROM contracts ct
					  JOIN service_rates sr ON sr.contract_id = ct.contract_id AND sr.customer_id = ct.customer_id
				      ...
				      GROUP BY ct.customer_id ) A ON A.customer_id = c.customer_id
					LEFT JOIN (
						SELECT customer_id, MIN(forecast_datetime) as forecast_datetime, SUM(forecast_ea) as forecast_ea
						FROM forecast_estimates
						WHERE supplier_id = $supplierID AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = '$algorithm'
						GROUP BY customer_id, DATE(forecast_datetime), HOUR(forecast_datetime)
					) fe_agg ON fe_agg.customer_id = c.customer_id
					$joinCT
					WHERE c.customer_status = 'Activ' AND c.supplier_id = $supplierID $cWhr
					ORDER BY HOUR(fe_agg.forecast_datetime), vco.fctOrder, c.customer_name, DAY(fe_agg.forecast_datetime)";
		}
	...
	private function selectHistoryValues($eStart, $eEnd, $estimationOptions, $exclPODs, $history = false, $lambda = 0.0, $v3 = false)
	{
		...
		if($estimationOptions->simple)
			$sql = $this->selectSimpleHistoryValues($eStart, $eEnd, $estimationOptions, $exclPODs, $history, $lambda, $v3);
		elseif($estimationOptions->temperature)
			$sql = $this->selectTemperatureHistoryValues($eStart, $eEnd, $estimationOptions, $exclPODs, $history, $lambda);
	...
	private function selectSimpleHistoryValues($eStart, $eEnd, $estimationOptions, $excludePODs, $history = false, $lambda = 0.0, $v3 = false)
	{
		log_message('debug', print_r($estimationOptions, true));

		if ($v3)
			return $this->selectSimpleHistoryValuesV3($eStart, $eEnd, $estimationOptions, $excludePODs, $history);

		if ($lambda > 0.0)
			return $this->selectSimpleHistoryValuesV2($eStart, $eEnd, $estimationOptions, $excludePODs, $history);

		return $this->selectSimpleHistoryValuesV1($eStart, $eEnd, $estimationOptions, $excludePODs, $history);
	}

	/** @deprecated Kept for reference; logic extracted to V1Algorithm and V2Algorithm traits */
	private function selectSimpleHistoryValues_legacy($eStart, $eEnd, $estimationOptions, $excludePODs, $history = false, $lambda = 0.0)
	{
		/*
		...
		*/
		...
		// v2: exact temporal cutoff (vs v1's year-boundary); EWMA weighted average vs simple average
		$temporalCutoff = $lambda > 0.0
			? "far.far_datetime < '$eStart'"
			: "year(far.far_datetime) < year('$eStart')";
		$aggFn = $lambda > 0.0
			? "SUM(A.far_ea * EXP(-$lambda * GREATEST(0, FLOOR(DATEDIFF('$eStart', A.far_datetime)/7)))) / NULLIF(SUM(EXP(-$lambda * GREATEST(0, FLOOR(DATEDIFF('$eStart', A.far_datetime)/7)))), 0)"
			: "AVG(A.far_ea)";
	...
	private function selectTemperatureHistoryValues($eStart, $eEnd, $estimationOptions, $excludePODs, $history = false, $lambda = 0.0)
	{
		/* 
			DAY MAPPING
		...
		*/
		...
		{
			// V2 cross-month: also include the last N weeks regardless of month
			$intervalWhrV2 = ($lambda > 0.0)
				? $this->v2CrossMonthClause($eStart, $intervalWhr)
				: $intervalWhr;
			...
			$sql = "INSERT INTO $rawDataTable
					...
					LEFT JOIN $tJOIN
					LEFT JOIN working_free_days wfd ON wfd.wfd_profile_id={$estimationOptions->wfdProfile} AND far.dt = wfd.free_date
					WHERE far.dt<'$eStart' AND ($intervalWhrV2) $podWhere $exclPODs $maxMonthsWhr
					GROUP BY far.pod, far.far_datetime";
						...
						( SELECT 
							...
							JOIN 
							 (SELECT 0 AS minute
							  ...
							  UNION SELECT 45) I
							ON fs.customer_id = $customerID 
							WHERE fs.synthetics_datetime >= DATE_ADD('$synthMaxMonths', INTERVAL 1 DAY) AND $synthIntervalWhr AND fs.synthetics_datetime < '$eStart'
							) far
						JOIN counties co ON co.county_code = 'B'						
						LEFT JOIN $tJOIN
						LEFT JOIN working_free_days wfd ON wfd.wfd_profile_id={$estimationOptions->wfdProfile} AND far.dt = wfd.free_date
		...
		// Zero-day filter: remove days where a POD's total consumption is 0 (missing-import artefacts)
		if ($lambda > 0.0) {
			$this->writeData("DELETE r FROM $rawDataTable r
				JOIN (SELECT pod, dt FROM $rawDataTable GROUP BY pod, dt HAVING SUM(far_ea) = 0) z
				ON r.pod = z.pod AND r.dt = z.dt");

			// V2 trend multiplier (per POD)
		...
		// v2: EWMA weighted aggregation for temperature path (only replaces AVG; MIN/MAX kept as-is)
		$weightExprTemp = "EXP(-$lambda * GREATEST(0, FLOOR(DATEDIFF('$eStart', far.far_datetime)/7)))";
		$aggExprTemp = ($lambda > 0.0 && $COMP === 'AVG')
			? "SUM(CAST(far.far_ea AS DECIMAL(20,8)) * $weightExprTemp) / NULLIF(SUM($weightExprTemp), 0)"
			: "$COMP(CAST(far.far_ea AS DECIMAL(20, 8)))";

		$temperatureMargin = $estimationOptions->temperature_margin ?? 0;
		$temperatureMarginMax = $estimationOptions->temperature_margin_max ?? 0;

		// V2 trend multiplier — applied per POD on the aggregated result
		$trendJoin   = ($lambda > 0.0) ? "LEFT JOIN tTrendMultipliers tm ON tm.pod = P.pod" : '';
		$trendFactor = ($lambda > 0.0) ? "* COALESCE(MAX(tm.multiplier), 1.0)"               : '';
		...
		$sql = "SELECT
				...
				tdates.datetime,
				'temperatura',
				CAST(SUM(COALESCE(A.far_ea,B.far_ea)) $trendFactor AS DECIMAL(20, 8)) AS far_ea
				FROM tdates_{$estimationOptions->wfdProfile} AS tdates
					...
					( SELECT 
						...
						JOIN 
						 (SELECT 0 AS minute
						  ...
						  UNION SELECT 45) I
						ON fs.customer_id = $customerID 
						WHERE fs.synthetics_datetime >= DATE_ADD('$maxMonths', INTERVAL 1 DAY) AND $synthIntervalWhr AND fs.synthetics_datetime < '$eStart'
						) far
	...
	private function filterEstimatedPODValues($eStart, $eEnd, $temp=false, $customerIDs = null)
	{
		...
		$cWhr = (!empty($customerIDs))
			? "AND fpe.customer_id IN (" . implode(',', array_map('intval', $customerIDs)) . ")"
			: "";
		...
		$sql = "SELECT CONCAT_WS(' / ','Filtrat',c.customer_name, fpe.pod, DATE(fpe.forecast_datetime), SUM(fpe.forecast_ea),
				...
				WHERE fpe.forecast_datetime >= '$eStart' AND fpe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
				$cWhr
	...
	private function insertEstimatedValues($eStart, $eEnd, $temp=false, $customerIDs = null, $algo = 'v1')
	{
		$cWhr = "";
		if($customerIDs != NULL)
		...
		if($temp)
		{
			$sql = "CREATE TEMPORARY TABLE IF NOT EXISTS forecast_temp_estimates AS SELECT forecast_datetime, cast(greatest(sum(forecast_ea),0) AS DECIMAL(20,8)) as forecast_ea FROM forecast_temp_pods_estimates
				WHERE $cWhr forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) and forecast_ea is not null GROUP BY customer_id, forecast_datetime";
		}
		else
			$sql = "INSERT INTO forecast_estimates (supplier_id, customer_id, forecast_datetime, forecast_ea, algorithm) SELECT supplier_id, customer_id, forecast_datetime, cast(greatest(sum(forecast_ea),0) AS DECIMAL(20,8)) as forecast_ea, '$algo' FROM forecast_pods_estimates
				WHERE $cWhr supplier_id = {$this->supplierID} AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND forecast_ea is not null GROUP BY customer_id, forecast_datetime
	...
	private function prepareTempTable($eStart, $eEnd, $customers)
	{
		...
		*/
		...
		$sql = "SELECT MAX(counter) as value FROM
				(SELECT count(*) as counter FROM forecast_temperatures WHERE temperature_datetime >= '$eStart' AND temperature_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) UNION
				SELECT count(*) as counter FROM weather_data WHERE layer = 'temp' AND county_code='B' AND weather_datetime >= '$eStart' AND weather_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) UNION
				SELECT count(*) as counter FROM weather_data WHERE layer = 'feelslike' AND county_code='B' AND weather_datetime >= '$eStart' AND weather_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)) X";
		log_message('debug',$sql);
	...
	function hasEstimates($customerID, $eStart, $eEnd)
	{
		$sql = "SELECT count(estimate_id)=(datediff('$eEnd','$eStart')+1) as value FROM (
						...
						WHERE fe.customer_id = $customerID AND fe.supplier_id = ".$this->supplierID." AND fe.forecast_datetime >= '$eStart' AND fe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
	...
	function estimate($eStart, $eEnd, $customers, $customerType)
	{
		...
		// Phase 1: collect non-manual customers and cache their options
		$customersToEstimate = [];
		$estimationOptionsMap = [];
		...
		foreach($customers as $customerID)
		{
			...
			$customersToEstimate[] = (int)$customerID;
			$estimationOptionsMap[$customerID] = $estimationOptions;
		}

		// Phase 2: batch DELETE for all customers before any INSERTs
		if(!empty($customersToEstimate))
		{
			$ids = implode(',', $customersToEstimate);

			$this->writeData("DELETE FROM forecast_pods_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)");

			$this->writeData("DELETE FROM forecast_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = 'v1' AND NOT EXISTS (SELECT 1 FROM forecast_customers_manual_estimates fcme WHERE fcme.supplier_id = {$this->supplierID} AND fcme.customer_id = forecast_estimates.customer_id AND fcme.date = DATE(forecast_estimates.forecast_datetime) AND fcme.date BETWEEN '$eStart' AND '$eEnd')");
		}

		// Phase 3: estimate and INSERT per customer
		$estimatedCustomers = [];

		foreach($customersToEstimate as $customerID)
		{
			...
			$estimationOptions = $estimationOptionsMap[$customerID];
			$pods = $this->getCustomersPODs($customerID);
			...
			foreach($pods as $pod)
			{
				...
				if(isset($estimationOptions->{$pod}))
				{
					...
					//check number of values — query inserted table directly instead of re-running the expensive $sqlValues
					$estimationIntervalSize = $this->getNumberOfDays($eStart, $eEnd) * 96;
					$checkSQL = "SELECT pod as value FROM forecast_pods_estimates WHERE supplier_id={$this->supplierID} AND customer_id=$customerID AND pod='$pod' AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND forecast_ea IS NOT NULL GROUP BY pod HAVING count(*) != $estimationIntervalSize";
			...
			if(!$this->hasSynthetics($eStart, $eEnd, $estimationOptions, $exclPODs))
			{
				//check number of values — query inserted table directly instead of re-running the expensive $sqlValues
				$estimationIntervalSize = $this->getNumberOfDays($eStart, $eEnd) * 96;
				$exclPODsFilter = !empty($exclPODs) ? "AND pod NOT IN ($exclPODs)" : "";
				$checkSQL = "SELECT pod as value FROM forecast_pods_estimates WHERE supplier_id={$this->supplierID} AND customer_id=$customerID AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND forecast_ea IS NOT NULL $exclPODsFilter GROUP BY pod HAVING count(*) != $estimationIntervalSize";
				$podErrors = $this->getValueArray($checkSQL);
				foreach($podErrors as $p)
					$this->errorCollection[] = $this->getCustomerName($customerID).':'.$p.': Estimare incompleta. Date insuficiente in istoric sau intervalul de selectie temperaturi/energie este prea ingust.';
			}
		...
		if(!empty($estimatedCustomers))
		{
			...
			$eV = $this->insertEstimatedValues($eStart, $eEnd, false, $estimatedCustomers, 'v1');
			$sql = "SELECT c.customer_name as value FROM customers c
					LEFT JOIN forecast_estimates fe ON fe.supplier_id = {$this->supplierID} AND fe.customer_id = c.customer_id AND fe.forecast_datetime >= '$eStart' AND fe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND fe.algorithm = 'v1'
			...
			$this->persistPodEstimates($eStart, $eEnd, $estimatedCustomers, 'v1');
			$this->errorCollection[] = $eV." valori estimate (v1)";

			$ids = implode(',', array_map('intval', $estimatedCustomers));

			if ($this->runExtraAlgorithms) {
			// === Phase v2: EWMA — re-run with λ=0.02, store as algorithm='v2' ===
			$this->writeData("DELETE FROM forecast_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = 'v2'");

			foreach($estimatedCustomers as $customerID)
			{
				$this->rawDataCounter = 0;
				$estimationOptions = $estimationOptionsMap[$customerID];
				$pods = $this->getCustomersPODs($customerID);
				...
				foreach($pods as $pod)
				{
					...
					if(isset($estimationOptions->{$pod}))
					{
						$sqlValues = $this->selectHistoryValues($eStart, $eEnd, $estimationOptions->{$pod}, '', false, 0.02);
						if(empty($sqlValues)) continue;
						$this->insertEstimatedPODValues($sqlValues);
						$exclPODs .= "'$pod',";
					}
				}
				$exclPODs = rtrim($exclPODs, ',');
				...
				$sqlValues = $this->selectHistoryValues($eStart, $eEnd, $estimationOptions, $exclPODs, false, 0.02);
				if(!empty($sqlValues))
					$this->insertEstimatedPODValues($sqlValues);
			}

			$this->filterEstimatedPODValues($eStart, $eEnd, false, $estimatedCustomers);
			$eV2 = $this->insertEstimatedValues($eStart, $eEnd, false, $estimatedCustomers, 'v2');
			$this->persistPodEstimates($eStart, $eEnd, $estimatedCustomers, 'v2');
			$this->errorCollection[] = $eV2." valori estimate (v2)";

			// === Phase v3: V2-style with HDD/CDD kernel (simple path) + lag-feature blend post-process ===
			$this->writeData("DELETE FROM forecast_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = 'v3'");
			...
			foreach($estimatedCustomers as $customerID)
			{
				...
				foreach($pods as $pod)
				{
					...
					if(isset($estimationOptions->{$pod}))
					{
						$sqlValues = $this->selectHistoryValues($eStart, $eEnd, $estimationOptions->{$pod}, '', false, 0.02, true /*v3*/);
						if(empty($sqlValues)) continue;
						$this->insertEstimatedPODValues($sqlValues);
						$exclPODs .= "'$pod',";
					}
				...
				$sqlValues = $this->selectHistoryValues($eStart, $eEnd, $estimationOptions, $exclPODs, false, 0.02, true /*v3*/);
				if(!empty($sqlValues))
					$this->insertEstimatedPODValues($sqlValues);
			}

			// Lag-feature blend post-process — applies to forecast_pods_estimates in-place
			$this->v3ApplyLagBlend($eStart, $eEnd, $estimatedCustomers);

			$this->filterEstimatedPODValues($eStart, $eEnd, false, $estimatedCustomers);
			$eV3 = $this->insertEstimatedValues($eStart, $eEnd, false, $estimatedCustomers, 'v3');
			$this->persistPodEstimates($eStart, $eEnd, $estimatedCustomers, 'v3');
			$this->errorCollection[] = $eV3." valori estimate (v3)";

			// === Phase v4: Chronos-2 (Amazon foundation model) self-hosted via local microservice ===
			if ($this->v4ServiceReady()) {
				$this->writeData("DELETE FROM forecast_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = 'v4'");

				$v4Rows = $this->v4EstimateCustomers($eStart, $eEnd, $estimatedCustomers);
				if ($v4Rows > 0) {
					$this->filterEstimatedPODValues($eStart, $eEnd, false, $estimatedCustomers);
					$eV4 = $this->insertEstimatedValues($eStart, $eEnd, false, $estimatedCustomers, 'v4');
					$this->persistPodEstimates($eStart, $eEnd, $estimatedCustomers, 'v4');
					$this->errorCollection[] = $eV4." valori estimate (v4 Chronos-2)";
				} else {
					$this->errorCollection[] = "v4 Chronos-2: 0 valori estimate (toate PODs au history insuficient sau call-uri esuate)";
				}
			} else {
				log_message('warning', 'V4 Chronos service not ready — phase skipped');
				$this->errorCollection[] = "v4 Chronos-2: serviciu indisponibil — faza omisa";
			}

			} // end if ($this->runExtraAlgorithms)

			// === Accuracy log: precompute monthly stats for whichever algorithms ran ===
			$this->computeAccuracyLog($eStart, $eEnd, $estimatedCustomers);
		}
		else
	...
	// Copy per-POD estimates from staging table to persistent forecast_pod_estimates.
	// Must be called after insertEstimatedValues, before v2 overwrites forecast_pods_estimates.
	private function persistPodEstimates($eStart, $eEnd, $customerIDs, $algo)
	{
		$ids  = implode(',', array_map('intval', $customerIDs));
		$algo = addslashes($algo);
		$this->writeData("DELETE FROM forecast_pod_estimates WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids) AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY) AND algorithm = '$algo'");
		$this->writeData("INSERT INTO forecast_pod_estimates (supplier_id, customer_id, pod, forecast_datetime, forecast_ea, algorithm)
			SELECT supplier_id, customer_id, COALESCE(pod,''), forecast_datetime, forecast_ea, '$algo'
			FROM forecast_pods_estimates
			WHERE supplier_id={$this->supplierID} AND customer_id IN ($ids)
			AND forecast_datetime >= '$eStart' AND forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
			AND forecast_ea IS NOT NULL");
	}

	// Return per-POD estimates for the EVR POD view.
	public function readAllPodEstimates($eStart, $eEnd, $customers, $intervalType, $algorithm = 'v1')
	{
		$supplierID = $this->supplierID;
		$cWhr = '';
		if (!empty($customers))
			$cWhr = "AND fpe.customer_id IN (" . implode(',', array_map('intval', $customers)) . ")";

		if ($intervalType === '15min') {
			$sql = "SELECT fpe.pod, fpe.forecast_datetime, fpe.forecast_ea
					FROM forecast_pod_estimates fpe
					WHERE fpe.supplier_id = $supplierID $cWhr
					AND fpe.forecast_datetime >= '$eStart' AND fpe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
					AND fpe.algorithm = '$algorithm'
					ORDER BY fpe.pod, fpe.forecast_datetime";
		} else {
			$sql = "SELECT fpe.pod, MIN(fpe.forecast_datetime) as forecast_datetime, SUM(fpe.forecast_ea) as forecast_ea
					FROM forecast_pod_estimates fpe
					WHERE fpe.supplier_id = $supplierID $cWhr
					AND fpe.forecast_datetime >= '$eStart' AND fpe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
					AND fpe.algorithm = '$algorithm'
					GROUP BY fpe.pod, fpe.customer_id, DATE(fpe.forecast_datetime), HOUR(fpe.forecast_datetime)
					ORDER BY fpe.pod, forecast_datetime";
		}

		$pods = $this->db->query(
			"SELECT DISTINCT fpe.pod as value FROM forecast_pod_estimates fpe
			 WHERE fpe.supplier_id = $supplierID $cWhr
			 AND fpe.forecast_datetime >= '$eStart' AND fpe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
			 AND fpe.algorithm = '$algorithm'
			 ORDER BY fpe.pod"
		)->getResultArray();
		$pods = array_column($pods, 'value');

		return ['data' => $this->getArray($sql), 'pods' => $pods];
	}

	// Precompute monthly accuracy metrics (MAPE, MAE, RMSE, bias) per customer per algorithm.
	// Joins forecast_estimates against the customer's actual customer_far_{id} table.
	// Upserts into forecast_accuracy_log.
	public function computeAccuracyLog($eStart, $eEnd, $customerIDs)
	{
		foreach($customerIDs as $customerID)
		{
			$tableExists = (int)$this->getValue("SELECT COUNT(*) as value FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'customer_far_$customerID'");
			if(!$tableExists) continue;

			$sql = "INSERT INTO forecast_accuracy_log
					(supplier_id, customer_id, algorithm, stat_month, n_intervals,
					 mape, mae, rmse, bias_pct, total_realized_kwh, total_estimated_kwh)
					SELECT
					    fe.supplier_id,
					    fe.customer_id,
					    fe.algorithm,
					    DATE_FORMAT(fe.forecast_datetime, '%Y-%m-01') AS stat_month,
					    COUNT(*) AS n_intervals,
					    AVG(CASE WHEN cf.far_ea > 0 THEN ABS(fe.forecast_ea - cf.far_ea) / cf.far_ea * 100 ELSE NULL END) AS mape,
					    AVG(ABS(fe.forecast_ea - cf.far_ea)) AS mae,
					    SQRT(AVG(POW(fe.forecast_ea - cf.far_ea, 2))) AS rmse,
					    AVG(CASE WHEN cf.far_ea > 0 THEN (fe.forecast_ea - cf.far_ea) / cf.far_ea * 100 ELSE NULL END) AS bias_pct,
					    SUM(cf.far_ea) AS total_realized_kwh,
					    SUM(fe.forecast_ea) AS total_estimated_kwh
					FROM forecast_estimates fe
					JOIN (
					    SELECT far_datetime, SUM(far_ea) AS far_ea
					    FROM customer_far_$customerID
					    WHERE far_datetime >= '$eStart' AND far_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
					    GROUP BY far_datetime
					) cf ON cf.far_datetime = fe.forecast_datetime
					WHERE fe.customer_id = $customerID
					AND fe.supplier_id = {$this->supplierID}
					AND fe.forecast_datetime >= '$eStart'
					AND fe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
					GROUP BY fe.customer_id, fe.algorithm, stat_month
					ON DUPLICATE KEY UPDATE
					    n_intervals = VALUES(n_intervals),
					    mape = VALUES(mape),
					    mae = VALUES(mae),
					    rmse = VALUES(rmse),
					    bias_pct = VALUES(bias_pct),
					    total_realized_kwh = VALUES(total_realized_kwh),
					    total_estimated_kwh = VALUES(total_estimated_kwh),
					    computed_at = CURRENT_TIMESTAMP";
			$this->writeData($sql);
		}
	}

	// Return monthly accuracy stats for selected customers/period/algorithms.
	// Used by the API for the accuracy log screen and CSV download.
	public function getAccuracyLog($customerIDs, $eStart, $eEnd, $algos = ['v1','v2','v3'])
	{
		$algoList = implode(',', array_map(fn($a) => "'".addslashes($a)."'", $algos));
		$cWhr = empty($customerIDs) ? '' : 'AND fal.customer_id IN ('.implode(',', array_map('intval', $customerIDs)).')';
		return $this->getArray("
			SELECT fal.*, c.customer_name
			FROM forecast_accuracy_log fal
			JOIN customers c ON c.customer_id = fal.customer_id
			WHERE fal.stat_month >= DATE_FORMAT('$eStart', '%Y-%m-01')
			AND fal.stat_month <= DATE_FORMAT('$eEnd', '%Y-%m-01')
			AND fal.algorithm IN ($algoList)
			$cWhr
			ORDER BY c.customer_name, fal.stat_month, fal.algorithm
		");
	}

	// Return raw (forecast_datetime, realized_ea, estimated_ea, error_pct) for a customer/period/algorithm.
	// Used for CSV download and detailed analysis.
	public function getAccuracyDetails($customerID, $eStart, $eEnd, $algorithm)
	{
		$tableExists = (int)$this->getValue("SELECT COUNT(*) as value FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'customer_far_$customerID'");
		if(!$tableExists) return [];
		$algo = addslashes($algorithm);
		return $this->getArray("
			SELECT
			    fe.forecast_datetime,
			    ROUND(cf.far_ea, 8) AS realized_ea,
			    ROUND(fe.forecast_ea, 8) AS estimated_ea,
			    CASE WHEN cf.far_ea > 0
			         THEN ROUND((fe.forecast_ea - cf.far_ea) / cf.far_ea * 100, 4)
			...
			WHERE fe.customer_id = $customerID
			AND fe.supplier_id = {$this->supplierID}
			AND fe.algorithm = '$algo'
			AND fe.forecast_datetime >= '$eStart'
			AND fe.forecast_datetime < DATE_ADD('$eEnd', INTERVAL 1 DAY)
			ORDER BY fe.forecast_datetime
		");
	}
	...
	function getEstimateTrend($eStart, $eEnd, $customers, $customerType)
	{
		...
		WHERE DATE(fe.forecast_datetime) BETWEEN '$eStart' AND '$eEnd' AND fe.algorithm = 'v1'
	...
	function getForecastReport($year, $month, $estimationType)
	{
		...
		foreach($customerIDs as $customerID)
		{
			...
			(SELECT CAST(SUM(fe.forecast_ea) AS DECIMAL(20,3)) FROM forecast_estimates fe WHERE fe.customer_id = $customerID AND year(fe.forecast_datetime) = $year AND MONTH(fe.forecast_datetime) = $month AND fe.algorithm = 'v1') AS forecast_ea,
			$farSQL
			$synthSQL
