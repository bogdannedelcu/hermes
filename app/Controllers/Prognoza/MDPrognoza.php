...
class MDPrognoza extends BaseController
{
	...
	public function Synthetics()
	{
		...
		$customerID = $_GET['customerID'] ?? $_SESSION['select-customer'] ?? 0;
		$cl->addTitle($this->syntheticsModel->getCustomerName($customerID)." / Sintetice ".ucfirst($formatter->format($this->data['date'])),$maxDays+1	,18);
		...
		$td = explode('-',$YMdate);
		$gData = $this->syntheticsModel->getSynthetics($td[1],$td[0], $customerID ?: null);
		$synthType = $this->syntheticsModel->getSyntheticsType($td[1],$td[0], $customerID ?: null);
