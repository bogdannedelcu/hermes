...
class ForecastModel extends Model
{
	...
	public function readEstimates($month, $year, $customers, $consumptionType,$intervalType)
	{
		...
		if(!empty($customers))
		{
			if($energy)
				...
				$sql = "SELECT day, hour, minute, SUM(forecast_ea) AS forecast_ea FROM (
					SELECT DAY(fc.forecast_datetime) as day, HOUR(fc.forecast_datetime) as hour, minute(fc.forecast_datetime) as minute, AVG(forecast_ea) AS forecast_ea FROM forecast_estimates fc 
					WHERE year(forecast_datetime)=$year AND MONTH(forecast_datetime)=$month AND customer_id IN ($customerIDs) AND supplier_id=$supplierID 
					GROUP BY fc.customer_id, $dateGroup) Z
					GROUP BY hour, day
					ORDER BY hour, day";				
		}
		else
		{
			if($energy)
				...
				$sql = "SELECT day, hour, minute, sum(forecast_ea) as forecast_ea FROM (
					...
					GROUP BY ct.customer_id ) A ON A.customer_id = fc.customer_id
					WHERE year(forecast_datetime)=$year and month(forecast_datetime)=$month and supplier_id=$supplierID
					group by fc.customer_id, HOUR(forecast_datetime), DAY(forecast_datetime)) Z
					GROUP BY hour, day
					ORDER BY hour, day";
		}
	...
	public function readAllEstimates($eStart, $eEnd, $customers,$consumptionTypeID, $intervalType)
	{
		...
		$sql = "SELECT c.customer_id, c.customer_name,fe.forecast_datetime, $f(fe.forecast_ea) as forecast_ea FROM customers c
	...
	public function convertTo15()
	{
		...
		// Step 1: Retrieve hourly data
		$sql = "SELECT * FROM forecast_pods_estimates 
				WHERE CONCAT(DATE(forecast_datetime), pod) IN (
					SELECT CONCAT(DATE(forecast_datetime), pod)
					FROM forecast_pods_estimates
					GROUP BY DATE(forecast_datetime), pod
					HAVING COUNT(*) <= 24)";
		$data = $this->getArray($sql);

		// Step 2: Convert hourly data to 15-minute intervals
		$sqlValues = "";$numRows=0;			
		foreach ($data as $row) {
			$forecast_ea_15min = $row['forecast_ea'] / 4; // Divide by 4 for 15-minute data
			
			for ($i = 0; $i < 4; $i++) {
				$numRows++;
				$forecast_datetime = date("Y-m-d H:i:s", strtotime($row['forecast_datetime'] . " + " . ($i * 15) . " minutes"));
				
				// Step 3: Insert new data
				$sqlValues .= "({$row['supplier_id']}, {$row['customer_id']}, '{$row['pod']}', '{$forecast_datetime}', '{$row['estimation_type']}', {$forecast_ea_15min}),";
				
				if($numRows>1000)
				{
					$sqlValues = substr($sqlValues,0,-1);
					$sql = "INSERT INTO forecast_pods_estimates (supplier_id, customer_id, pod, forecast_datetime, estimation_type, forecast_ea) VALUES 
					$sqlValues 
		...
		// Step 1: Retrieve hourly data grouped by customer_id
		$sql = "SELECT * FROM forecast_estimates 
				WHERE CONCAT(DATE(forecast_datetime), customer_id) IN (
					SELECT CONCAT(DATE(forecast_datetime), customer_id)
					FROM forecast_estimates
					GROUP BY DATE(forecast_datetime), customer_id
					HAVING COUNT(*) <= 24)";
		$stmt = $this->getArray($sql);
		...
		foreach ($stmt as $row) {
			...
			for ($i = 0; $i < 4; $i++) {
				...
				// Step 3: Insert new data into forecast_estimates
				$sqlValues .= "({$row['supplier_id']}, {$row['customer_id']}, '{$forecast_datetime}', {$forecast_ea_15min}),";
				if($numRows>1000)
				{
					$sqlValues = substr($sqlValues,0,-1);
					$sql = "INSERT INTO forecast_estimates (supplier_id, customer_id, forecast_datetime, forecast_ea) VALUES 
	...
	{			
		...
		if(!empty($customerType) && $customerType != -1)
		{
			...
			//		  JOIN forecast_consumption_types fct ON fct.fct_id = fcct.fct_id AND fct.consumption_type_name='$customerType' OR (fcct.fcct_id IS NULL AND fct.consumption_type_name='General' AND fct.consumption_type_name='$customerType')";
	...
	{
		/*
		$sql = 'SELECT fcct.pod, fcto.option_name, fcto.value FROM customers c
				LEFT JOIN forecast_customers_consumption_types fcct ON fcct.customer_id = c.customer_id 
	...
	private function selectSimpleHistoryValues($eStart, $eEnd, $estimationOptions, $excludePODs, $history = false)
	{
		...
		*/
		...
		{
			if($this->hasSynthetics($eStart, $eEnd, $estimationOptions, $excludePODs, 'absolut'))				
				$synthIntervalWhr = $this->getDateRangeWhere($customerID,$supplierID, $rawStart, $rawEnd, $marginDays, true);
			else
	...
	private function selectTemperatureHistoryValues($eStart, $eEnd, $estimationOptions, $excludePODs, $history = false)
	{
		...
		*/
		...
		/*
		$interval = \DateTime::createFromFormat('Y-m-d H:i:s',$rawDataIntervalEnd.' 24:00:00')->diff(\DateTime::createFromFormat('Y-m-d H:i:s',$rawDataIntervalStart.' 00:00:00'));
		$intervalHours = $interval->days * 24 + $interval->h;
		log_message('error','tcount'.$this->countTemperatures($rawDataIntervalStart.' 00:00:00',$rawDataIntervalEnd.' 23:00:00'));
		log_message('error','intervalHours'.$intervalHours);
		
		//$temperatures = $this->getTemperatures($rowDataIntervalStart.' 00:00:00',$rowDataIntervalEnd.' 23:00:00');
		//if( empty($temperatures) OR count($temperatures) != $intervalHours )
		if($this->countTemperatures($rawDataIntervalStart.' 00:00:00',$rawDataIntervalEnd.' 23:00:00') != $intervalHours )
		{
			$this->error = "Nu exista temperaturi pentru toate datele istorice!";
			return '';
		}
		*/	
	...
	private function hasSynthetics($eStart, $eEnd, $estimationOptions, $exclPODs, $synthType='absolut')
	{
		...
		if($estimationOptions->ignore_synthetics) return false;
	...
	{	
		$cWhr = "";
		if($customerIDs != NULL)
			$cWhr .= " customer_id IN (".implode(',',$customerIDs).") AND ";
	...
	{	
		log_message("error","Estimates started...");	
		...
		if(!empty($estimatedCustomers))
		{
			...
			$sql = "SELECT c.customer_name as value FROM customers c 
					LEFT JOIN forecast_estimates fe ON fe.customer_id = c.customer_id AND date(fe.forecast_datetime) BETWEEN '$eStart' AND '$eEnd' 
					WHERE c.customer_id IN (".implode(',',$estimatedCustomers).")
					GROUP BY c.customer_id
	...
	function getEstimateTrend($eStart, $eEnd, $customers, $customerType)
	{
		...
		{
			$cWhr = "AND c.customer_id IN (". implode(',',$customers) .")";
			$fcWhr = "AND far.customer_id IN (". implode(',',$customers) .")";
		}
