...
class DataSourceResult {
    ...
    private function total($tableName, $properties, $request) {
        //if ((isset($request->filter) && is_array($request->filter) && count($request->filter)>0) || (isset($request->filter) && isset($request->filter->operator))) {
		...
		log_message("error",print_r($statement,true));
        $statement->execute();
    ...
    private function mergeSortDescriptors($request) {
        ...
        // Groups must come first in ORDER BY so PHP groupBy() sees consecutive equal values.
        // User sorts are secondary (within each group).
        return array_merge($groups, $sort);
    }
    ...
    private function calculateAggregates($table, $aggregates, $request, $propertyNames) {
        ...
        if (count($aggregates) > 0) {
            ...
            for ($index = 0; $index < $count; $index++) {
                ...
                $functions[] = $name.'('.$aggregate->field.') as '.$aggregate->field.'_SEP_'.$aggregate->aggregate;
            }
    ...
    private function bindFilterValues($statement, $filter) {
        ...
        for ($index = 0; $index < count($filters); $index++) {
			...
			/*$date = date_parse($value);*/ //why is not used?
	...
	public function getTableColumns($table_name)
	{
		...
		AND TABLE_NAME = ? AND column_comment!=\'hidden\'';
		
		$columns = [];
		foreach ($this->query($sql,[$table_name])['data'] as $d)
		{		
	...
	public function getTableColumnsDataTypes($table_name)
	{
		$sql = "SELECT column_name, data_type, 
		case when character_maximum_length IS NOT NULL then character_maximum_length
		  when NUMERIC_PRECISION IS NOT NULL then CONCAT(NUMERIC_PRECISION,',',coalesce(numeric_scale,''))
		  END AS size
		FROM INFORMATION_SCHEMA.COLUMNS 
		WHERE TABLE_SCHEMA = Database()
		AND TABLE_NAME = ? AND column_comment!='hidden'";
		...
		{		
			...
			array_push($columns,$d['data_type']);
	...
	public function query($sql,$input_parameters=[]) {
		...
		$result['total'] = $statement->rowCount();
		$status = $statement->errorInfo();
	...
	public function getWhereStr($table, $properties, $request) {

        $propertyNames = $this->propertyNames($properties);	
			
		if (isset($request->filter)) {
            $wStr = $this->filter($properties, $request->filter);
        }
		else return '';
				
		$filters = array();
        $this->flatten($filters,  $request->filter);
        ...
        for ($index = 0; $index < count($filters); $index++) {
            ...
            if ($operator == 'contains' || $operator == 'doesnotcontain') {
			 ...
			 $wStr = str_replace(":filter$index","'$value'",$wStr);
		}
		
		return $wStr;
    }
	
    public function read($table, $properties, $request = null, $distinct=false) {
        $result = array();
		...
		if($distinct)
			$sql = sprintf('SELECT distinct %s FROM %s', implode(', ', $propertyNames), $table);
		else
