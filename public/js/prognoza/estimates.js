...
function onPrezentGetFreeDays(response)
{
	...
	{
		r.push("R2C"+ (response.freeDays[i]+1),"R3C"+ (response.freeDays[i]+1),"R"+viewSize+"C"+ (response.freeDays[i]+1));
	}
...
function fillPrezentViewEstimates(response)
{
	...
	addTotalRow('Total',numDays,'SUM','0.000');
	sheet.range("A29").formula("=SUM(R4C2:R"+(viewSize+3)+"C"+(numDays+1)+")").background("rgb(167,214,255)").color("black").bold(true).textAlign("center").format("0.000");
...
function fillSummaryViewEstimates(response)
{
	...
	{	
		...
		addTotalRow('Total',customers.length*intervalSize,"SUM","0.000",5).fontSize(fontSize);
		sheet.range("R"+(currRow-1)+"C1").formula("=SUM(B1:AF27)").background("rgb(167,214,255)").color("black").bold(true).textAlign("center").format("0.000").fontSize(fontSize);
